...
class ProximityAlertRecord:
    ...
    This records a machine-generated candidate relationship that has been saved
    to the SQLite database for durable review. Unlike ProximityAlert, this
    class represents stored state: it has a pa_id, lifecycle status,
    timestamps, and stable references to the two chunks involved.

    If the user confirms this alert, kurrent creates a ConfirmedLink that
...
class ProximityAlert:
    """A generated candidate relationship between two chunks.

    This is the in-memory object returned by ProximityAlerter. It represents a
    possible semantic connection discovered during the current alerting run,
    enriched with text, page, and path information for display/review.

    It is not itself a persisted database row. If kurrent stores this candidate
    for durable review, it should be converted into a ProximityAlertRecord.
    """
    source_chunk_id: str
    target_chunk_id: str
    distance: float
